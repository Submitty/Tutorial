...
def init_outgoing_connection(name):
  ...
  server_address = (name, int(outgoing_port))
  sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
...
def send_message_and_wait_for_response(server_name, message):
  ...
  while response == None:
    response, return_port = check_for_request()
    time.sleep(.1)
  print(response)

def auto_run_one():
  delay_seconds(1)
  send_message_and_wait_for_response('docker_A', 'ping')
...
def auto_run_three():
  send_message_and_wait_for_response('docker_A','not ping')
...
def auto_run_four():
  while True:
    send_out_message('docker_A','ping')

def auto_run_five():
  message = "not ping"
  message = message * 10000  
  send_message_and_wait_for_response('docker_A', message)
